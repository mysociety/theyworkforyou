...
from mysociety import config  # noqa:E402
...
from lords.resolvenames import lordsList  # noqa:E402
from resolvemembernames import memberList  # noqa:E402
...
class Entry(object):
    ...
    def __init__(self, event):
        ...
        chamber = event[
            "Type"
        ]  # Select & Joint Committees, General Committee, Grand Committee, Main Chamber, Westminster Hall
        ...
        self.link_calendar = CALENDAR_LINK % {
            "place": house_url,
            "iso": self.event_date,
        }
    ...
    def update_people(self, delete_old=True):
        ...
        if delete_old:
            db_cursor.execute(
                "DELETE FROM future_people where calendar_id = %s", (self.id,)
            )
...
for new_entry in get_calendar_events():
    ...
    if row_count:
        ...
        old_tuple = (
            ...
            + (
                (
                    (datetime.datetime.min + old_row[7]).time().isoformat()
                    if old_row[7] is not None
                    else None
                ),
            )
            + (
                (
                    (datetime.datetime.min + old_row[8]).time().isoformat()
                    if old_row[8] is not None
                    else None
                ),
            )
            + old_row[9:]
