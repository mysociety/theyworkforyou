...
class MEMBER {
    ...
    var $canonical = true;
    var $lord_entered_reason;
    var $lord_entered_house;
    var $mp_left_reason;
    var $mp_left_house;
    var $mp_left_constituency;
	...
	function MEMBER ($args) {
		...
		if (isset($args['member_id']) && is_numeric($args['member_id'])) {
			// Get the person_id for this term in office.
			$person_id = $this->member_id_to_person_id($args['member_id']);

		} elseif (isset($args['name'])) {
			if (isset($args['constituency'])) {
				$person_id = $this->name_to_person_id($args['name'], $args['constituency']);
			} else {
				$person_id = $this->name_to_person_id($args['name']);
			}
		...
		$q = $this->db->query("SELECT	member_id,
								house,
								title,
								first_name,
								last_name,
		...
		if ($q->rows() > 0) {
			...
			$this->person_id	= $q->field(0, 'person_id');
		} else {
			$this->valid = false;
			return;
		}
		
		// Now, we might have fetched more than one row here, in which case
		// this person has had more than one term in office.
		...
		if ($q->rows() > 1) {
			...
			for ($row=1; $row<$q->rows(); $row++) {
				...
				if (!$transfer && $this->house == 2 && $q->field($row, 'house') == 1) {
					...
					$this->mp_left_reason = $q->field($row, 'left_reason');
					$this->mp_left_constituency = $q->field($row, 'constituency');
				}
				...
				if ($entered_time < $member_entered_time && $q->field($row, 'house') != 3) {
					...
					$this->entered_reason = $q->field($row, 'entered_reason');
				} 
				...
				if ($member_left_time != '-1' && $left_time > $member_left_time ||
					...
					$this->left_house = $q->field($row, 'left_house');
					$this->left_reason = $q->field($row, 'left_reason');
					// We want the most recent constituency.
					$this->constituency = $q->field($row, 'constituency');
	...
	function name_to_person_id ($name, $const='') {
		...
		if ($const || $this_page=='peer') {
			$const = ' AND constituency=\''.mysql_escape_string($const)."'";
		}
		$q .= $const . ' ORDER BY left_house DESC';
		$q = $this->db->query($q);
		if ($q->rows > 1) {
...
#                    str_replace("/person/", "/person/biography/", $guardian_url);
	...
	function full_name($no_mp_title = false) {
		if ($no_mp_title && $this->house() == 1) {
			return member_full_name($this->house(), '', $this->first_name, $this->last_name, $this->constituency);
		} else {
			return member_full_name($this->house(), $this->title, $this->first_name, $this->last_name, $this->constituency);
		}
	}
	function house() 			{ return $this->house; }
	...
	function entered_house() 	{ return $this->entered_house; }
	...
	function left_house() 		{ return $this->left_house; }
	...
	function url($absolute = true) {
		$member_url = make_member_url($this->full_name(true), $this->constituency(), $this->house());
		if ($this->house() == 1)
			$URL = new URL('mp');
		else
			$URL = new URL('peer');
	...
	function display () {
		...
		$member = array (
			...
			'entered_reason'	=> $this->entered_reason_text($this->entered_reason),
			'left_house'		=> $this->left_house(),
			'left_house_text'	=> $this->left_house_text($this->left_house),
			'left_reason'		=> $this->left_reason_text($this->left_reason),
			'current_member'	=> $this->current_member(),
            'full_name'			=> $this->full_name(),
            'the_users_mp'		=> $this->the_users_mp(),
			'lord_entered_house'		=> $this->lord_entered_house,
			'lord_entered_house_text'	=> $this->entered_house_text($this->lord_entered_house),
	...
	function previous_mps() {
		$previous_people = '';
		$q = $this->db->query('SELECT DISTINCT(person_id), first_name, last_name FROM member WHERE house=1 AND constituency = "'.$this->constituency() . '" AND person_id != ' . $this->person_id() . ' AND entered_house < "' . $this->entered_house() . '"');
		for ($r = 0; $r < $q->rows(); $r++) {
			$pid = $q->field($r, 'person_id');
			$name = $q->field($r, 'first_name') . ' ' . $q->field($r, 'last_name');
			$previous_people .= '<li><a href="/mp/?pid='.$pid.'">'.$name.'</a></li>';
		}
		...
		if ($this->person_id() == 10218)
			$previous_people = '<li><a href="/mp/?pid=10341">Oona King</a></li>';
	...
	function future_mps() {
		...
		for ($r = 0; $r < $q->rows(); $r++) {
			$pid = $q->field($r, 'person_id');
			$name = $q->field($r, 'first_name') . ' ' . $q->field($r, 'last_name');
			$future_people .= '<li><a href="/mp/?pid='.$pid.'">'.$name.'</a></li>';
		}
