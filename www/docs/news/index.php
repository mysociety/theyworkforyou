...
if (preg_match("#/(\d\d\d\d)/(\d\d)/(\d\d)/([a-z0-9_]+)(\.php)?$#", $uri, $matches)) {
    ...
    foreach ($all_news as $id => $news_row) {
        ...
        print "<p>Posted on " . \MySociety\TheyWorkForYou\Utility\DateTime::formatDate(substr($date, 0, 10), LONGDATEFORMAT) . " at " . substr($date, 11);
    ...
    $DATA->set_page_metadata($this_page, 'title', \MySociety\TheyWorkForYou\Utility\DateTime::formatDate("$year-$month-01", "F Y"));
    $PAGE->page_start();
    $PAGE->stripe_start();
