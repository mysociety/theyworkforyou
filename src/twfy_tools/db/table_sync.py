"""
Synchronize an importer's complete snapshot with its database rows.

Callers supply model instances for all rows they own within a partition.
Missing owned keys are deleted; new keys are created and
changed keys updated. An empty snapshot removes the owned subset.
...
def sync_table(
    ...
    *,
    ...
    same key, but are never deleted. Both filters use Django queryset kwargs.

    For English organizations, pass key_field="org_id",
    partition={"language": "en"}, owned={"loader__in": ["posts-uk"]}, and all
    desired English rows for that loader.
    """
