    ...
    try:
        ...
        for schema_file in schemas:
            if len(schema_file.strip()) > 0:
                run_ssh_test("( cd "+configuration['DEPLOYED_PATH']+" && mysql twfy -u twfy < "+schema_file+" )",
                             user="alice",
        ...
        run_ssh_test("cd "+configuration['DEPLOYED_PATH']+"/scripts/ && ./xml2db.pl --members --from=2009-10-01 --to=2009-10-31",
                     test_name="Importing the member data",
                     test_short_name="import-member-data")

        # Import the rest of the data:

        run_ssh_test("cd "+configuration['DEPLOYED_PATH']+"/scripts/ && ./xml2db.pl --wrans --debates --westminhall --wms --lordsdebates --ni --scotland --scotwrans --scotqs --standing  --from=2009-10-01 --to=2009-10-31",
