...
class Member {
    ...
    public function member_id_to_person_id($member_id) {
        ...
        if ($q->rows > 0) {
            ...
            throw new MemberException('Sorry, there is no member with a member ID of "' . _htmlentities($member_id) . '".');
        }
    }
    ...
    public function postcode_to_person_id($postcode, $house=null) {
        ...
        $constituency = strtolower(Utility\Postcode::postcodeToConstituency($postcode));
    ...
    public function constituency_to_person_id($constituency, $house=null) {
        global $PAGE;
        if ($constituency == '') {
            throw new MemberException('Sorry, no constituency was found.');
        }
        ...
        if ($q->rows > 0) {
            return $q->field(0, 'person_id');
                ...
                throw new MemberException('Sorry, there is no current member for the "' . _htmlentities(ucwords($constituency)) . '" constituency.');
        }
    }
    ...
    public function name_to_person_id($name, $const='') {
        global $PAGE, $this_page;
        if ($name == '') {
            throw new MemberException('Sorry, no name was found.');
        }
        ...
        if ($this_page=='peer') {
            ...
            if (!$success)
                $success = preg_match('#^(.*?) (.*?)()$#', $name, $m);
            if (!$success) {
                throw new MemberException('Sorry, that name was not recognised.');
            }
            $params[':title'] = $m[1];
    ...
    or (title=:first_name AND first_name=:middle_name AND last_name=:last_name)
        ...
        if ($q->rows > 1) {
            ...
            throw new MemberException('Sorry, there is no current member with that name.');
            return false;
        }
    }
