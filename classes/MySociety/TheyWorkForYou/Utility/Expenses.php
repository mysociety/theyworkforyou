 ...
 * Expenses Utilities
 *
 * Utility functions related to expenses
 */

class Expenses
{

    public static function displayTable($extra_info, $gadget=false, $start_year=9) {
            $out = '';
            $latest_year = 9;
            $earliest_year = 2;
    ...
    expenses</a></p>";
      ...
      $out .= '<tbody>';
        ...
        $out .= self::row('col1', $extra_info,1, $gadget, $start_year, $end_year, $med_year, $wide_year);
        $out .= '</tr><tr><td class="row-2 left">London costs</td>';
        $out .= self::row('col2', $extra_info,2, $gadget, $start_year, $end_year, $med_year, $wide_year);
        $out .= '</tr><tr><td class="row-1 left">Office running costs</td>';
        $out .= self::row('col3', $extra_info,1, $gadget, $start_year, $end_year, $med_year, $wide_year);
        $out .= '</tr><tr><td class="row-2 left" >Staffing costs</td>';
        $out .= self::row('col4', $extra_info,2, $gadget, $start_year, $end_year, $med_year, $wide_year);
        $out .= '</tr><tr><td class="row-1 left">Communications Allowance</td>';
        $out .= self::row('colcomms_allowance', $extra_info,1, $gadget, $start_year, $end_year, $med_year, $wide_year);
        $out .= '</tr><tr><td class="row-2 left">Members\' Travel</td>';
        $out .= self::row('col5', $extra_info,2, $gadget, $start_year, $end_year, $med_year, $wide_year);
        $out .= '</tr><tr><td class="row-1 left">Members\' Staff Travel</td>';
        $out .= self::row('col6', $extra_info,1, $gadget, $start_year, $end_year, $med_year, $wide_year);
        $out .= '</tr><tr><td class="row-2 left">Members\' Spouse Travel</td>';
        $out .= self::row('colspouse_travel_a', $extra_info,2, $gadget, $start_year, $end_year, $med_year, $wide_year);
        $out .= '</tr><tr><td class="row-1 left">Members\' Family Travel</td>';
        $out .= self::row('colfamily_travel_a', $extra_info,1, $gadget, $start_year, $end_year, $med_year, $wide_year);
        $out .= '</tr><tr><td class="row-2 left">Centrally Purchased Stationery</td>';
        $out .= self::row('col7', $extra_info,2, $gadget, $start_year, $end_year, $med_year, $wide_year);
        $out .= '</tr><tr><td class="row-1 left">Stationery: Associated Postage Costs</td>';
        $out .= self::row('col7a', $extra_info,1, $gadget, $start_year, $end_year, $med_year, $wide_year);
        $out .= '</tr><tr><td class="row-2 left">Centrally Provided Computer Equipment</td>';
        $out .= self::row('col8', $extra_info,2, $gadget, $start_year, $end_year, $med_year, $wide_year);
        $out .= '</tr><tr><td class="row-1 left">Other Costs</td>';
        $out .= self::row('col9', $extra_info,1, $gadget, $start_year, $end_year, $med_year, $wide_year);
        $out .= '</tr><tr><th class="left total">Total</th>';
        $out .= self::row('total', $extra_info,2, $gadget, $start_year, $end_year, $med_year, $wide_year);
        $out .= '</tr></tbody></table>';

            if (isset($extra_info['expenses2009_colmp_reg_travel_a']) and $extra_info['expenses2009_col5'] > 0 and $start_year
    >= 9 and $end_year <= 9){
                $out .= self::extraTravel($extra_info, 2009);
            }

            if (isset($extra_info['expenses2008_colmp_reg_travel_a']) and $extra_info['expenses2008_col5'] > 0 and $start_year >=
    8 and $end_year <= 8) {
                $out .= self::extraTravel($extra_info, 2008);
            }
    ...
    public static function row($col, $extra_info, $style, $gadget, $start_year, $end_year, $med_year, $wide_year) {
        $out = '';
        ...
        for ($ey=$start_year; $ey>=$end_year; --$ey) {
               $extra_class = '';
           list($amount, $rank, $extra) = self::row($ey, $col, $extra_info, $gadget);
           if (!$amount) $amount = '&nbsp;';
               $rowspan = '';
               if ($col=='col7' && $ey==2009) {
    ...
    public static function item($ey, $col, $extra_info, $gadget) {
        if ($col=='col7' && $ey==2009) {
            $col = 'colstationery';
        }
        $k = 'expenses' . $ey . '_' . $col;
    ...
    public static function extraTravel($extra_info, $year) {
    ...
    public static function yearString($year) {
      return '200' . ($year-1) . '/0' . $year;
    }

    public static function mostRecent($extra_info, $gadget=false) {
      $out = '<div id="expenses-header"> Expenses ';
      $year = '';
      for ($ey=2009; $ey>=2002; --$ey) {
        if (isset($extra_info['expenses'.$ey.'_col1'])) {
          $out .= self::yearString($ey-2000);
          $out .= '</div>';
      ...
      if ($year == '')
        ...
        for ($i=1; $i<=11; $i++) {
            ...
            $row = self::row($year, $r, $extra_info, $gadget);
